...
# @5.2 Create a function similar to those above to set top_score
def set_top_score(new_score):
    """ Change the value of 'top_score' """
    global top_score
    top_score = new_score

def add_coords(a, b):
    """ Take two sets of coordinates (p,q) and (r,s) and return (p+q, r+s) """
    p,q = a
    r,s = b
    return (p+r, q+s)
...
class Snake():
    ...
    def next_space(self, direction=None):
        ...
        Argument: direction (optional) - the direction the snake will move in.
                                         ...
                                         direction is used.
        """
        # Add the specified direction to the head of the list.
        if direction == None:
    ...
    def grow(self):
        ...
        # will move to next and add this to the front of the position list
    ...
    def shrink(self):
        ...
        # Use 'slice' notation (:) to take elements from the start up to but
        # not including the last one (-1)
        self.position[:] = self.position[:-1]

    def move(self):
        """ Move the snake one space """
        # @1.3 Use grow() and shrink() to add one square to the head then
    ...
    def change_direction(self, new_direction):
        ...
        """
        # @2.1 Update self.direction with the new direction specified. Only do
        # this if moving in the new direction doesn't take us back to where we
        # just came from. (The previous location is now the second item in the
        # position list.)
        if self.length() < 2 or self.next_space(new_direction) != self.position[1]:
...
def current_score():
    ...
    # Current score is equal to the snake's current length minus it's starting
    # length of 5.
...
def crashed(next_space):
    ...
    return (not grid.within_grid(next_space)) or (next_space in snake)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # change the snake's direction accordingly.
...
def draw_splash_screen():
    ...
    draw_text('Use the arrow keys to control the snake', y=200)
...
def draw_game_over_screen():
    ...
    if current_score() == top_score:
        draw_text('You got the high score!', font=large_font, y=100, color=BLUE)
    # Switch to the new screen
    pygame.display.flip()
...
def draw_game_screen():
    ...
    draw_text('Top Score: {}'.format(top_score), x=350, y=30, color=WHITE)
    # Switch to the new screen
    pygame.display.flip()
